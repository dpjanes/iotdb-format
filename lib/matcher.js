...
const matcher = (template, paramd) => {
    ...
    template = template.replace(/{{(.*?)}}/g, function(match, variable) {
        ...
        const mapped = paramd.patterns[variable]

        if (_.is.Undefined(mapped)) {
            throw new Error(`expected to see "patterns[${variable}]" defined`)
        }
    ...
    return s => {
        const result = _.d.clone(paramd.otherwise || {})
        result._matched = false

        const match = s.match(rex)
        if (match) {
            // no eligant answer for empty string
            if (match.index === 0) {
                result._matched = true
            }

            variables.forEach((variable, x) => {
                const value = match[x + 1]
                if (!_.is.Empty(value)) {
                    _.d.set(result, variable, value)
                    result._matched = true
                }
            })
        }
